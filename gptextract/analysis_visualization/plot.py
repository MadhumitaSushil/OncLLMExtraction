...
# Load data from CSV
df = pd.read_csv('../../output/results_overall.csv')
...
group2 = ['Symptoms Datetime', 'SymptomsAtDiagnosis Datetime', 'SymptomsDueToCancer Datetime']
...
fig, axs = plt.subplots(nrows=len(group_titles), ncols=3, figsize=(30, 30))
...
for i, group in enumerate(groups):
    ...
    axs[i][0].barh([j - 0.3 for j in x], group_df['BLEU4_GPT-4'], height=0.2, label='GPT4', color='#82AAE3')
    axs[i][0].barh([j - 0.1 for j in x], group_df['BLEU4_GPT3.5'], height=0.2, label='GPT3.5', color='#91D8E4')
    axs[i][0].barh([j + 0.1 for j in x], group_df['BLEU4_FLANUL2'], height=0.2, label='FLAN-UL2', color='#BFEAF5')

    axs[i][1].barh([j - 0.3 for j in x], group_df['ROUGE1_GPT-4'], height=0.2, label='GPT4', color='#ff5252')
    axs[i][1].barh([j - 0.1 for j in x], group_df['ROUGE1_GPT3.5'], height=0.2, label='GPT3.5', color='#ff7b7b')
    axs[i][1].barh([j + 0.1 for j in x], group_df['ROUGE1_FLANUL2'], height=0.2, label='FLAN-UL2', color='#ffbaba')

    axs[i][2].barh([j - 0.3 for j in x], group_df['EM_F1_GPT-4'], height=0.2, label='GPT4', color='#ef820d')
    axs[i][2].barh([j - 0.1 for j in x], group_df['EM_F1_GPT3.5'], height=0.2, label='GPT3.5', color='#eb9605')
    axs[i][2].barh([j + 0.1 for j in x], group_df['EM_F1_FLANUL2'], height=0.2, label='FLAN-UL2', color='#ffbf00')

    # Set the x-axis tick labels and y-axis labels
    axs[i][0].set_yticks(x)
    axs[i][1].set_yticks(x)
    axs[i][2].set_yticks(x)
    axs[i][0].set_yticklabels(labels, fontsize=16)
    axs[i][1].set_yticklabels(labels, fontsize=16)
    axs[i][2].set_yticklabels(labels, fontsize=16)

    axs[i][0].set_xticks([0, 0.2, 0.4, 0.6, 0.8, 1])
    axs[i][1].set_xticks([0, 0.2, 0.4, 0.6, 0.8, 1])
    axs[i][2].set_xticks([0, 0.2, 0.4, 0.6, 0.8, 1])
